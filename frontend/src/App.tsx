...
type AuthFormView = "signup" | "login" | "forgotPassword" | "none";

function App() {
  const [allNotesData, setAllNotesData] = useState<Note[]>([]); // This will store ALL notes for tag generation
  const [rawNotes, setRawNotes] = useState<Note[]>([]); // This will store the notes to be displayed
  const [currentView, setCurrentView] = useState<NoteView>("all");
  const [activeTag, setActiveTag] = useState<string>("");
  const [allTags, setAllTags] = useState<string[]>([]); // This will now hold ALL unique tags from allNotesData
  ...
  // Use a useCallback for fetching all user notes to prevent re-creation
  // This fetches ALL notes (non-archived and archived) to derive ALL unique tags
  const fetchAllUserNotesForTags = useCallback(async () => {
    if (loggedInUser) {
      try {
        const nonArchived = await fetchNotes();
        const archived = await fetchArchived();
        // Combine all notes to get a comprehensive list for tag extraction
        const combinedNotes = [...nonArchived, ...archived];
        setAllNotesData(combinedNotes); // This state is for ALL notes to generate tags
      } catch (error) {
        console.error("Error fetching all notes for tags:", error);
        setAllNotesData([]);
      }
    } else {
      setAllNotesData([]); // Clear notes if no user
    }
  }, [loggedInUser]);

  // Effect to load the *displayed* notes based on `currentView`
  // This should filter from `allNotesData` or fetch specifically for the view
  useEffect(() => {
    async function loadDisplayedNotes() {
      if (!loggedInUser) {
        setRawNotes([]); // Clear displayed notes if not logged in
        return;
      }

      let notesToDisplay: Note[] = [];
      const allCurrentNotes = await fetchNotes();
      if (currentView === "all") {
        notesToDisplay = allCurrentNotes; // Fetch current non-archived notes
      } else if (currentView === "archived") {
        notesToDisplay = await fetchArchived(); // Fetch current archived notes
      } else if (currentView === "search") {
        const lowerCaseSearchInput = searchInput.toLowerCase();

        notesToDisplay = allCurrentNotes.filter((each) => {
          const titleMatch = each.title
            .toLowerCase()
            .includes(lowerCaseSearchInput);
          const textMatch = each.text
            .toLowerCase()
            .includes(lowerCaseSearchInput);
          const tagMatch = each.tags
            .toString()
            .toLowerCase()
            .includes(lowerCaseSearchInput);
        ...
        // For tags, filter from the potentially complete list, or refetch non-archived and filter
        // Fetch non-archived notes
        notesToDisplay = allCurrentNotes.filter((each) =>
          each.tags.includes(currentView)
        );
      }

      setRawNotes(notesToDisplay); // Set the notes that will be displayed in NotesSection
    }
    loadDisplayedNotes();
  }, [currentView, loggedInUser, searchInput]); // Depend on currentView and loggedInUser

  // Effect to generate all unique tags from `allNotesData`
  useEffect(() => {
    const tagList = allNotesData.flatMap((each) => each.tags || []);
    const uniqueTagSet = new Set(tagList);
    const uniqueTagList = Array.from(uniqueTagSet);
    setAllTags(uniqueTagList);
  }, [allNotesData]); // This now only depends on the full list of notes for tags

  // Trigger initial fetch of all notes for tags when user logs in
  useEffect(() => {
    fetchAllUserNotesForTags();
  }, [fetchAllUserNotesForTags]);
  ...
  const getHeadingTag = () => {
    ...
    if (activeTag) {
      return (
        <p className="text-f-neutral-600">
          Notes Tagged:<span className="text-f-neutral-950"> {activeTag}</span>
        </p>
      );
    }
    ...
    if (currentView === "search") {
      return `Showing results for ${searchInput}`;
    }
    return "Notes";
  };
  ...
  async function handleLogout() {
    try {
      ...
      setRawNotes([]); // Clear displayed notes
      setAllNotesData([]); // Clear all notes data (for tags)
      setAllTags([]); // Clear tags
      setShowForm(false); // Close any open forms
      setSelectedNote(null); // Deselect any note
      // Ensure settings view is closed
    } catch (error) {
      console.error("There is an error during logout", error);
      alert("There was an error during logout");
    }
  }

  // console.log('cuurent view is :',currentView)

  return (
    <>
      {authFormView === "none" ? (
        <div className="  h-screen flex flex-col bg-f-neutral-100 lg:bg-white dark:bg-f-neutral-950   ">
          <div className="    flex h-full overflow-hidden flex-1  ">
           

            <div className=" w-full     flex flex-col    ">
              <div className="hidden lg:flex items-center">
                <div className="text-f-neutral-950 h-full dark:text-f-white py-[28px] px-[32px] lg:w-[272px]  ">
                  <svg
                ...
                <div className=" flex-1 border-l-2 border-f-neutral-200 dark:border-f-neutral-800">
                  <NavbarFinal
                    headingTag={getHeadingTag()}
                    onClicked={() =>
                      setCurrentView(
                        currentView === "settings" ? "all" : "settings"
                      )
                    }
              ...
              <div className=" block lg:hidden text-f-neutral-950 dark:text-f-white px-[32px] py-[23px] lg:w-[272px]   ">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    width="95"
                    height="28"
              ...
              <div className="flex overflow-hidden  h-full">
                <div className=" px-[16px]  hidden lg:block overflow-y-auto no-scrollbar border-r-2 border-f-neutral-200 dark:border-f-neutral-800  ">
                  <div className="flex flex-col  pb-2 gap-1 border-b-2 border-f-neutral-200 dark:border-f-neutral-800 ">
                    <button
                      className={`flex gap-2 items-center text-f-neutral-950 dark:text-f-neutral-0 text-preset-4 lg:w-[240px] p-[12px] rounded-[8px] cursor-pointer hover:bg-f-neutral-100 hover:dark:bg-f-neutral-800 ${
                        currentView === "all" ? "bg-f-neutral-100 dark:bg-f-neutral-800" : ""
                      } `}
                      onClick={() => {
                        setCurrentView("all");
                  setActiveTag(""); // Clear active tag when viewing all notes
                        setSearchInput("");
                      }}
                    >
                      <House
                        className={`w-[20px] h-auto ${
                          currentView === "all" ? "text-f-blue-500" : "text-f-neutral-950 dark:text-white"
                        }`}
                      />{" "}
                      All Notes{" "}
                      {currentView === "all" ? (
                        <ChevronRight className="ml-auto dark:text-f-neutral-0 h-[18px] " />
                      ) : null}
                    </button>
                    <button
                      className={`flex gap-2 items-center text-f-neutral-950 dark:text-f-neutral-0 text-preset-4 lg:w-[240px] p-[12px] rounded-[8px] cursor-pointer hover:bg-f-neutral-100 hover:dark:bg-f-neutral-800 ${
                        currentView === "archived" ? "bg-f-neutral-100 dark:bg-f-neutral-800" : ""
                      } `}
                      onClick={() => {
                        setCurrentView("archived");
                        setActiveTag(""); // Clear active tag when viewing archived notes
                        setSearchInput("");
                      }}
                    >
                      <ArchiveRestore
                        className={`w-[20px] h-auto ${
                          currentView === "archived" ? "text-f-blue-500" : "text-f-neutral-950 dark:text-white"
                        }`}
                      />{" "}
                      Archived Notes{" "}
                  ...
                  <p className="text-preset-4 text-f-neutral-500 pt-2 pb-2">
                    Tags
                  </p>
                  <div className="gap-1 flex flex-col w-24 ">
                    {allTags.map((each, index) => (
                      <button
                        key={index}
                        className={`flex gap-2 justify-start items-start text-f-neutral-950 dark:text-white text-preset-4 lg:w-[240px] p-[12px] rounded-[8px] cursor-pointer hover:bg-f-neutral-100 hover:dark:bg-f-neutral-800 ${
                          activeTag === each ? "bg-f-neutral-100 dark:bg-f-neutral-800" : ""
                        } `}
                        onClick={() => handleTag(each)}
                      >
                        <Tag
                          className={`w-[20px] h-auto ${
                            activeTag === each ? "text-f-blue-500" : ""
                          }`}
                        />{" "}
                        {each}
                        {activeTag === each ? (
                ...
                <div className="flex-1 bg-white dark:bg-f-neutral-950  rounded-t-3xl px-[32px] py-[24px] lg:px-0 lg:py-0 ">
                  {currentView === "settings" && (
                    <Settings onLogoutSuccessfull={handleLogout}></Settings>
                  )} 

                  {
                   currentView !== 'settings' && currentView !== 'tags' &&  (
                   ...
                   {/* {
                    currentView === 'search' && (
                      ...
                      <div className="flex gap-2 w-full pl-[18px] py-[13px] pr-[16px] border-1 border-f-neutral-300 dark:border-f-neutral-600 rounded-[8px]">
                    ...
                    <NotesSection
                    headingTag={getHeadingTag()}
                      rawNotes={rawNotes} // Pass the filtered notes to NotesSection
                      setRawNotes={setRawNotes} // Keep this for NotesSection to update its own state
                      // You might need to add a prop to NotesSection to tell it to trigger a full refresh of allNotesData
                      // For example: onNoteChange={fetchAllUserNotesForTags}
                      setCurrentView={setCurrentView}
        ...
        <div className="flex justify-center items-center h-svh w-full bg-f-neutral-100 dark:bg-f-neutral-700">
          {authFormView === "signup" && (
            <SignupForm
              onDismiss={() => setAuthFormView("none")}
              onSignupSuccessfull={(user) => {
                console.log("user Signedup:", user);
                setAuthFormView("login");
              }}
              onShowLogin={() => setAuthFormView("login")}
            ></SignupForm>
          )}
          {authFormView === "login" && (
            <LoginForm
              ...
              onShowSignup={() => setAuthFormView("signup")}
